...
class VW_Tour_Lite_About_Widget extends WP_Widget {
	function __construct() {
		parent::__construct(
			'VW_Tour_Lite_About_Widget',
			__('VW About us', 'vw-tour-lite'),
			array( 'description' => __( 'Widget for about us section in sidebar', 'vw-tour-lite' ), ) 
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		<aside class="widget">
	        ...
	        if(!empty($title) ){ ?><h3 class="custom_title"><?php echo esc_html($instance['title']); ?></h3><?php } ?>
		        ...
		        <?php if(!empty($read_more_url) ){ ?><div class="more-button"><a class="custom_read_more" href="<?php echo esc_url($instance['read_more_url']); ?>"><?php if(!empty($read_more_text) ){ ?><?php echo esc_html($instance['read_more_text']); ?><?php } ?><i class="fas fa-angle-right"></i></a></div><?php } ?>
	        <?php echo '</div>';
			?>
		</aside>
		<?php
	...
	public function form( $instance ) {
    	...
    	<p>
        <label for="<?php echo esc_attr($this->get_field_id('author')); ?>"><?php esc_html_e('Author Name:','vw-tour-lite'); ?></label>
    	...
    	<p>
        <label for="<?php echo esc_attr($this->get_field_id('designation')); ?>"><?php esc_html_e('Designation:','vw-tour-lite'); ?></label>
    	...
    	<p>
        <label for="<?php echo esc_attr($this->get_field_id('description')); ?>"><?php esc_html_e('Description:','vw-tour-lite'); ?></label>
    	...
    	<p>
		<label for="<?php echo esc_attr($this->get_field_id('read_more_text')); ?>"><?php esc_html_e('Button Text:','vw-tour-lite'); ?></label>
		...
		<p>
		<label for="<?php echo esc_attr($this->get_field_id('read_more_url')); ?>"><?php esc_html_e('Button Url:','vw-tour-lite'); ?></label>
		...
		<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'upload_image' )); ?>"><?php esc_html_e( 'Image Url:','vw-tour-lite'); ?></label>
		<?php
...
function vw_tour_lite_about_custom_load_widget() {
	register_widget( 'VW_Tour_Lite_About_Widget' );
}
add_action( 'widgets_init', 'vw_tour_lite_about_custom_load_widget' );